...
After scaffolding the project, copy the fonts into the project. The fonts live in the `fonts/` folder next to this SKILL.md file, inside `.agents\skills\dataminer-frontend\`.

```powershell
$skillFonts = Join-Path (Get-Item "<workspace-root>\.agents\skills\dataminer-frontend\fonts") FullName
...
- Only use these icons. You can find all available icon labels/ligatures in the `.agents\skills\dataminer-frontend\references\icon-ligatures.md` file.
...
- Use the `.agents\skills\dataminer-frontend\references\dataminer-logo.svg` for the logo shown at the left side of the header. Make sure the color of the logo also adapts to the theme.
