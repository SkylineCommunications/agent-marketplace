...
> **Skill reference notice:** This skill refers to additional skills that are not included in this public distribution: `dataminer-browser-automation`, `dataminer-qaops`. If any of them is required for the current task, inform the user and ask whether to continue while ignoring the missing skill or stop.
...
- **First, check for a saved sign-in.** If a Playwright storage-state file exists (default `~/.copilot/playwright-auth/dataminer-storage-state.json`), the browser can be started with it and no manual step is needed — load the `dataminer-browser-automation` skill, which owns that setup, then come straight back here to continue debugging.
- If there is no saved sign-in (or it has expired), send this message to the user:
...
Or, to avoid this every time, I can set up a reusable sign-in — see the `dataminer-browser-automation` skill.
...
| Task | Skill |
|------|-------|
| Reusable DataMiner sign-in for headless Playwright, plus screenshot conventions | `dataminer-browser-automation` |
| Building or restyling the UI once the bug is understood | `dataminer-frontend` |
| Verifying behavior on a real DataMiner with automated tests instead of the browser | `dataminer-qaops` |
